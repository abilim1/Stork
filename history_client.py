...
def pull_history(
    ...
    Raises RuntimeError on API / import failure.
    """
    try:
        import pandas as pd
        from moomoo import RET_OK, AuType, OpenQuoteContext
    except Exception as exc:
        raise RuntimeError(f"Import failed: {exc}. pip install moomoo-api pandas") from exc
    ...
    try:
        ...
        while True:
            ...
            if ret != RET_OK:
                raise RuntimeError(f"API error page {page}: {data}")
